Shader "Resonai/PreviewGizmo" {
   Properties {
      [HideInInspector] _MainTex ("Texture Image", 2D) = "white" {}
			[HideInInspector] _Scale("Scale", Float) = 0.4
         ...
         vertexOutput vert(vertexInput input) 
         {
            vertexOutput output;
						...
						float scale_with_obj_scale = base_scale_with_aspect * float4(model_scale_x, model_scale_y, 1, 1);
						float4 vertex_pos = float4(input.vertex.x, input.vertex.y, 0, 0.0) * base_scale_with_aspect;
